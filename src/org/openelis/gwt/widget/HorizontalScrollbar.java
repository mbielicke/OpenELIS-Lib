...
public class HorizontalScrollbar extends Composite implements HasScrollHandlers {
	
	/**
	 * Native GWT widget we are wrapping
	 */
	protected NativeHorizontalScrollbar bar;
	
	/**
	 * Reference to this object for firing in anonymous class
	 */
	protected HorizontalScrollbar source = this;
	...
	/**
	 * Returns the max amount the scroll bar can be scrolled.
	 * @return
	 */
	public int getScrollMax() {
		return bar.getScrollWidth();
	}
	...
	protected void setScrollPosition(int pos, boolean fire) {
		...
		if(pos < 0)
			pos = 0;
		else if(pos > bar.getMaximumHorizontalScrollPosition())
			pos = bar.getMaximumHorizontalScrollPosition();
		...
		scrolled = true;
		bar.setHorizontalScrollPosition(pos);
	}
